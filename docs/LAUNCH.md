...
| | State |
|---|---|
| Web | https://xorr-solana.vercel.app — hosted, Solana fork, `main` |
| iOS | v1.0.0 (build 1), bundle `finance.xorr.app`. Release build runs on the iOS simulator against the hosted executor and fork. |
...
- **Not on a fork.** The hosted build trades test money on a mainnet fork. A real launch needs the executor on Solana mainnet (`XORR_CHAIN=solana-mainnet`, `ALLOW_MAINNET=yes`), a paid RPC, real keys held in a KMS/HSM, and the venue vault removed from the owner-signed sale path.
...
- [ ] Refresh the fork right before recording (routes drift): redeploy `solana-fork` (see the deploy notes in `docs/RUNBOOK.md` / the fork Dockerfile).
