...
import { KNOWN_CHAINS, moneyOn, networkName } from './money.js';
...
describe('the two sides know the same chains', () => {
  it('the app knows every chain the executor does', () => {
    ...
    for (const key of KNOWN_CHAINS) {
      expect(app.CHAIN_KEYS, key).toContain(key);
    }
  });

  it('and the executor knows every chain the app does', () => {
    for (const key of app.CHAIN_KEYS) expect(KNOWN_CHAINS, key).toContain(key);
  });
});
...
describe('and the same facts about them', () => {
  it('agrees on what money on each chain is', () => {
    ...
    // `evm/money.ts` for exactly this reason; a disagreement here is a build calling a mainnet a test net.
    for (const key of KNOWN_CHAINS) {
      expect(app.moneyOnChain(key), key).toBe(moneyOn(key));
    }
  });
  ...
  it('names each chain the same way inside a sentence', () => {
    ...
    for (const key of KNOWN_CHAINS) {
      expect(app.chainSentenceNameOf(key), key).toBe(networkName(key));
    }
  });
});
