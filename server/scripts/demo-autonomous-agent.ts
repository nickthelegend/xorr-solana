 ...
 *
 * Demonstrates the full autonomous flow with xStocks-specific grounding:
 * 1. Corporate action schedule & multiplier check (Token-2022 Scaled UI)
 * 2. On-chain oracle vs Nasdaq extended-hours pricing (off-hours slippage guard)
 * 3. Strategy selection (best setup across momentum, event-driven, dca, grid)
 * 4. Non-custodial spend evaluation and on-chain Jupiter execution via guardAndSpend
 * 5. Automatic exit rule attachment (stop-loss / take-profit)
 * 6. Instant notification alerts on entry, exit, and kill
 */
import { evaluateBestSetup } from '../src/bot/autonomous.js';
...
import { getNasdaqSession, evaluateOffHoursGuard } from '../src/market/nasdaq.js';
...
async function main() {
  ...
  for (const [symbol, info] of Object.entries(XSTOCKS)) {
    ...
    console.log(
      `   • ${symbol.padEnd(6)}: Mint=${info.mint.slice(0, 8)}... | Live Mark=$${price?.toFixed(2)} | Scaled Multiplier=${multiplier.toFixed(1)}x`,
    );
  }

  console.log('\n2. GROUNDING SIGNALS (CORPORATE ACTIONS & NASDAQ SESSION)...');
  const session = getNasdaqSession();
  console.log(`   [Nasdaq Session]: ${session.session.toUpperCase()} (${session.phase} - ${session.easternTime}) | Exchange Open: ${session.isExchangeOpen}`);

  for (const symbol of Object.keys(XSTOCKS)) {
    const mark = (await stockPriceUsd(symbol)) ?? 200;
    const ca = await assessCorporateAction(symbol);
    const guard = evaluateOffHoursGuard({ symbol, onChainPrice: mark });

    console.log(`   • ${symbol}:`);
    console.log(`     - Corporate Actions: ${ca.imminentAction ? `${ca.imminentAction.kind.toUpperCase()} (${ca.imminentAction.title}) in ${ca.daysUntil}d [${ca.recommendation}]` : 'None imminent'}`);
    console.log(`     - Off-Hours Guard:   Spread=${(guard.spreadPct * 100).toFixed(2)}% | Action=${guard.action.toUpperCase()} | Suggested Slippage=${guard.suggestedSlippageBps} bps`);
  }

  console.log('\n3. EVALUATING CANDIDATE STRATEGIES (Propose & Decide Layer)...');
  const bestSetup = await evaluateBestSetup();
  if (!bestSetup) {
    console.error('   ❌ No viable setup found.');
    return;
  }
  ...
  console.log('\n4. SIMULATING AUTONOMOUS EXECUTION TICK...');
  ...
  console.log('\n5. DISPATCHING NOTIFICATIONS & LOGGING TO CHAT DRAWER...');
  ...
  console.log(
    `   ✔ Entry Alert Sent: "xorr: ${bestSetup.personaName} Traded - Bought ${demoUnits.toFixed(4)} ${bestSetup.symbol} ($25.00 at $${bestSetup.currentPrice.toFixed(2)}) via ${bestSetup.strategyKind}."`,
  );
  ...
  console.log(
    `   ✔ Exit Alert Sent: "xorr: Position Closed - Closed ${demoUnits.toFixed(4)} ${bestSetup.symbol} at $${bestSetup.targetPrice.toFixed(2)} (P&L: +$${(demoUnits * (bestSetup.targetPrice - bestSetup.currentPrice)).toFixed(2)})."`,
  );
