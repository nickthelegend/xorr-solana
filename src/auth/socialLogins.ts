 ...
 * the executor knows a person by their Privy id — never by their email — so a Google, X or GitHub account is as complete
 * an account as an emailed code makes. `twitter` is the id Privy has always used for X; the label is what people call it.
 *
...
export const SOCIAL_LOGINS: readonly { id: SocialProvider; label: string }[] = [
  ...
  { id: 'twitter', label: 'X' },
  // GitHub (2026-09-25): the same Privy account and embedded wallet as the others, for the people who build on Solana.
